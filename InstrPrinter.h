...
public:
  ...
  // New function to print instruction with surrounding context
  void printInstrWithContext(InstrRef instr, int numInstrsBefore = 2, 
                             int numInstrsAfter = 2, int numBlocksUp = 1) {
    // Get the block containing this instruction
    BlockRef block = instr.getBlock();
    if (!block) {
      printInstr(instr);
      return;
    }
    
    // Find the position of the instruction in the block
    auto it = std::find(block.begin(), block.end(), instr);
    if (it == block.end()) {
      printInstr(instr);
      return;
    }
    
    // Calculate the range of instructions to print
    auto startIt = it;
    auto endIt = it;
    
    // Move backward to get numInstrsBefore instructions
    for (int i = 0; i < numInstrsBefore && startIt != block.begin(); ++i) {
      --startIt;
    }
    
    // Move forward to get numInstrsAfter instructions
    for (int i = 0; i < numInstrsAfter && endIt != block.end(); ++i) {
      ++endIt;
    }
    
    // Print the surrounding context
    str << "// Block with " << block.size() << " instructions\n";
    
    // Print blocks upwards
    if (numBlocksUp > 0) {
      auto currentBlock = block;
      for (int i = 0; i < numBlocksUp && currentBlock.getParent(); ++i) {
        currentBlock = currentBlock.getParent();
        str << "// Parent block " << i + 1 << " with " << currentBlock.size() 
            << " instructions\n";
      }
    }
    
    // Print the instructions in range
    for (auto printIt = startIt; printIt != endIt; ++printIt) {
      if (printIt == it) {
        str << "> ";
      } else {
        str << "  ";
    ...
    str << "// Total instructions in block: " << block.size() << "\n";
  }
};
