...
private:
  ...
  void lowerMul(InstrRef instr) {
    // Handle general multiplication by implementing a simple multiplier
    ...
    auto bits = *lhs.getNumBits();
    if (bits == 0)
      report_fatal_error("Invalid bit width for multiplication");
      
    // Create accumulator
    HWValue acc = cbuild.zeroLike(lhs).get();
    
    // For each bit of the multiplier (rhs), if it's set, add the multiplicand (lhs) shifted appropriately
    for (uint32_t i = 0; i < bits; i++) {
      auto bit = build.buildSplice(rhs, 1, i);
      auto shiftedLhs = build.buildSLL(lhs, cbuild.val(bits, i).get());
      acc = build.buildAdd(acc, build.buildMux(bit, shiftedLhs, cbuild.zeroLike(lhs).get()));
    }
    
    instr.def(0)->as<WireRef>().replaceAllUsesWith(acc);
    destroyMap[instr] = 1;
  }
  ...
  void runOnInstr(InstrRef instr) {
    ...
    case *OP_MUL:
      if (config.lowerConstantMul && instr.other(1)->is<ConstantRef>())
  ...
  void run() {
    auto isLoweringInstr = [&](InstrRef instr) {
      ...
      if (instr.isOpc(OP_MUL))
        return config.lowerConstantMul || true; // Always lower MUL
