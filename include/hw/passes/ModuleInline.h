...
class ModuleInlinePass : public Pass<ModuleInlinePass> {
  ...
  void inlineInstance(InstanceIRef instance) {
    ...
    auto modToInline = instance.mod().iref();
    ...
    auto &sourceLocInfo = ctx.getCtx<CoreDialectContext>().instrSourceLocInfo;

    auto addNames = [&](RegisterRef srcReg, RegisterRef dstReg) {
      for (auto nm : regNameInfo.getNames(srcReg)) {
        auto instNm = instance.nameRaw();
        auto newNm = instNm ? (*instNm + '.' + nm) : nm;
        regNameInfo.addName(dstReg, newNm);
      };
    };
    auto addResetValue = [&](RegisterRef srcReg, RegisterRef dstReg) {
      if (regResetValue.inRange(srcReg) && regResetValue[srcReg]) {
        // for outputs give inner reset value priority, else parent.
        // todo: maybe diagnostic?
        if (!regResetValue.inRange(dstReg) || !regResetValue[dstReg] ||
            srcReg.iref().isOpc(HW_OUTPUT_REGISTER_DEF)) {
          auto val = regResetValue[srcReg];
          regResetValue.get_ensure(dstReg) = val;
        }
      }
    };
    ...
    auto inlineHook = [&](DeepCopier *self, InstrRef src,
      ...
      if (src.isOpc(HW_INPUT_REGISTER_DEF, HW_OUTPUT_REGISTER_DEF,
        ...
        addNames(srcReg, portReg);
        addResetValue(srcReg, portReg);
        regTypeInfo.copyType(srcReg, portReg);
        sourceLocInfo.copyDebugInfo(srcReg.iref(), portReg.iref());
        return true;
      }
      ...
      if (src.isOpc(HW_INSTANCE)) {
        ...
        if (auto *name = instr.as<InstanceIRef>().nameRaw()) {
          *name = std::string(instance.as<InstanceIRef>().name()) + '.' + *name;
        }
